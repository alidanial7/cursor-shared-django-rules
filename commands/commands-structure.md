...
```python
self.stdout.write(
    self.style.ERROR(f"❌ Error creating items: {e}")
)
```
...
- **Write clear comments** for each logical section of the command

## Comments Guidelines

Comments are **mandatory** in commands. They help with:

- Understanding command flow at a glance
- Identifying phases during code review
- Maintaining consistency across commands
...
def handle(self, *args, **options):
    ...
    try:
        ...
        if not items:
            self.stdout.write("⏭️ Skipping. No items to process.")
            return
        ...
        # Phase 3: Skip (expected condition)
        self.stdout.write("⏭️ Skipping. Already processed.")
...
- Mark each phase clearly (`# Phase 1: Starting`)
- Describe business logic sections (`# Validate prerequisites`)
